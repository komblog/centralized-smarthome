...
begin
  ...
  begin
    dtModul.ExecSQL('SELECT USR_ID, USR_PASSWORD, USR_LEVEL'#13 +
        'FROM USER'#13 + 'WHERE USR_NAME = "%s"', [edUsername.Text],
      dtModul.sqlQuery1);
    ...
    begin
      ...
      begin
        dtModul.ExecSQL('SELECT USR_ID, USR_NAME'#13 + 'FROM USER'#13 +
            'WHERE USR_LEVEL = 1', [], dtModul.sqlQuery2);
        dtModul.sqlQuery2.First;
        iBaris := 0;
        formMain.ClearTreeView;
        while not dtModul.sqlQuery2.Eof do
        begin
          formMain.TreeView1.Items.AddChild
            (nil, '[' + dtModul.sqlQuery2.Fields[0].AsString + ']' +
          ...
          dtModul.ExecSQL('SELECT OBJ_ID, OBJ_NAME'#13 + 'FROM OBJECT'#13 +
              'WHERE OBJ_USR_ID = %d', [dtModul.sqlQuery2.Fields[0].AsInteger],
            dtModul.sqlQuery3);
          dtModul.sqlQuery3.First;
          while not dtModul.sqlQuery3.Eof do
          begin
            formMain.TreeView1.Items.AddChild
              (formMain.TreeView1.Items[iNode], '(' + dtModul.sqlQuery3.Fields
                [0].AsString + ') ' + dtModul.sqlQuery3.Fields[1].AsString);
      ...
      begin
        dtModul.ExecSQL('SELECT OBJ_ID, OBJ_NAME'#13 + 'FROM OBJECT'#13 +
            'WHERE OBJ_USR_ID = %d', [__iUserID], dtModul.sqlQuery3);
        dtModul.sqlQuery3.First;
        while not dtModul.sqlQuery3.Eof do
        begin
          formMain.TreeView1.Items.AddChild
            (nil, '(' + dtModul.sqlQuery3.Fields[0].AsString + ') ' +
              dtModul.sqlQuery3.Fields[1].AsString);
          dtModul.sqlQuery3.Next;
        end;
