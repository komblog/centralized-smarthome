...
begin
  ...
  begin
    dtModul.ExecSQL('UPDATE MAX_POWER'#13 + 'SET MAX_POWER = %d',
      [StrToInt(edMaxPower.Text)], dtModul.sqlQuery1);
  end
  else
  begin
    dtModul.ExecSQL('UPDATE MAX_POWER'#13 + 'SET MAX_POWER = %d'#13 +
        'WHERE CLUSTER = %d', [StrToInt(edMaxPower.Text), StrToInt
        (cbCluster.Text)], dtModul.sqlQuery1);
  end;
  close;
end;
...
begin
  ...
  begin
    dtModul.ExecSQL('SELECT MAX_POWER FROM MAX_POWER'#13 +
        'WHERE CLUSTER = %d', [StrToInt(cbCluster.Text)], dtModul.sqlQuery1);
...
begin
  ...
  dtModul.ExecSQL('SELECT CLUSTER FROM MAX_POWER', [], dtModul.sqlQuery1);
