...
begin
  ...
  myObj.WriteInteger('SETTINGS', 'power' + IntToStr(i + 1), StrToInt
      (editPower.Text));
  myObj.WriteInteger('SETTINGS', 'status' + IntToStr(i + 1), 0);
  myObj.WriteInteger('SETTINGS', 'USR_ID' + IntToStr(i + 1),
    formLogin.__iUserID);
  myObj.Free;

  dtModul.ExecSQL(
    'INSERT INTO OBJECT (OBJ_ID, OBJ_NAME, OBJ_CLASS, OBJ_USR_ID, OBJ_POWER)'#13
      + 'VALUES'#13 + '(%d, "%s", %d, %d, %d)', [i + 1, editNamaObject.Text,
    StrToInt(objId.Text), formLogin.__iUserID, StrToInt(editPower.Text)],
    dtModul.sqlQuery1);
  Close;
