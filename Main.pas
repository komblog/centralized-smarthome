...
begin
  myReg := TIniFile.Create(ExtractFilePath(Application.ExeName)
      + 'Settings.ini');
  i := 1;
  posisi := 0;
  NextGrid1.ClearRows;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        NextGrid1.Cell[2, posisi - 1].AsString := myReg.ReadString
          ('SETTINGS', 'name' + IntToStr(i), 'Object_' + IntToStr(i));
        NextGrid1.Cell[3, posisi - 1].AsInteger := myReg.ReadInteger
          ('SETTINGS', 'status' + IntToStr(i), 0);
        NextGrid1.Cell[4, posisi - 1].AsFloat := myReg.ReadInteger
          ('SETTINGS', 'power' + IntToStr(i), 0) * NextGrid1.Cell[3, posisi - 1]
          .AsInteger / 100;
      end
      ...
      begin
        ...
        NextGrid1.Cell[2, i - 1].AsString := myReg.ReadString
          ('SETTINGS', 'name' + IntToStr(i), 'Object_' + IntToStr(i));
        NextGrid1.Cell[3, i - 1].AsInteger := myReg.ReadInteger
          ('SETTINGS', 'status' + IntToStr(i), 0);
        NextGrid1.Cell[4, i - 1].AsFloat := myReg.ReadInteger
          ('SETTINGS', 'power' + IntToStr(i), 0) * NextGrid1.Cell[3, i - 1]
          .AsInteger / 100;
...
var
  ...
  sRule_Id, sRule_Obj_In, sRule_Obj_In_Name, sRule_Obj_In_Status,
    sRule_Obj_Out, sRule_Obj_Out_Name, sRule_Obj_Out_Status,
    sRule_User_Id: string;
  bFound : boolean;
begin
  ...
  else if sMsg = 'END' then
  begin
    IdTCPClientUpdate.Disconnect;
  end
...
//    begin
//      myRule.EraseSection(ruleSectionList[i]);
//      Inc(i);
//    end;

    sRule_Id := Copy(sMsg, Pos('@', sMsg) + 1, Pos('#', sMsg) -
        (Pos('@', sMsg) + 1));
    sRule_Obj_In := Copy(sMsg, Pos('#', sMsg) + 1, Pos('^', sMsg) -
        (Pos('#', sMsg) + 1));
    sRule_Obj_In_Name := Copy(sMsg, Pos('^', sMsg) + 1, Pos('$', sMsg) -
        (Pos('^', sMsg) + 1));
    sRule_Obj_In_Status := Copy(sMsg, Pos('$', sMsg) + 1, Pos('%', sMsg) -
        (Pos('$', sMsg) + 1));
    sRule_Obj_Out := Copy(sMsg, Pos('%', sMsg) + 1, Pos('+', sMsg) -
        (Pos('%', sMsg) + 1));
    sRule_Obj_Out_Name := Copy(sMsg, Pos('+', sMsg) + 1, Pos('*', sMsg) -
        (Pos('+', sMsg) + 1));
    sRule_Obj_Out_Status := Copy(sMsg, Pos('*', sMsg) + 1, Pos('&', sMsg) -
        (Pos('*', sMsg) + 1));
    sRule_User_Id := Copy(sMsg, Pos('&', sMsg) + 1, MaxInt);

    if sRule_Obj_In_Status = '100' then
    begin
      myRule.WriteString('!' + sRule_Obj_In_Name + '_ON@' + sRule_Id,
        'Rule_ID', sRule_Id);
      myRule.WriteString('!' + sRule_Obj_In_Name + '_ON@' + sRule_Id,
        'Rule_Obj_In', sRule_Obj_In);
      myRule.WriteString('!' + sRule_Obj_In_Name + '_ON@' + sRule_Id,
        'Rule_Obj_In_Name', sRule_Obj_In_Name);
      myRule.WriteString('!' + sRule_Obj_In_Name + '_ON@' + sRule_Id,
        'Rule_Obj_In_Status', sRule_Obj_In_Status);
      myRule.WriteString('!' + sRule_Obj_In_Name + '_ON@' + sRule_Id,
        'Rule_Obj_Out', sRule_Obj_Out);
      myRule.WriteString('!' + sRule_Obj_In_Name + '_ON@' + sRule_Id,
        'Rule_Obj_Out_Name', sRule_Obj_Out_Name);
      myRule.WriteString('!' + sRule_Obj_In_Name + '_ON@' + sRule_Id,
        'Rule_Obj_Out_Status', sRule_Obj_Out_Status);
      myRule.WriteString('!' + sRule_Obj_In_Name + '_ON@' + sRule_Id,
        'Rule_User_Id', sRule_User_Id);
    end
    else if sRule_Obj_In_Status = '0' then
    begin
      myRule.WriteString('!' + sRule_Obj_In_Name + '_OFF@' + sRule_Id,
        'Rule_ID', sRule_Id);
      myRule.WriteString('!' + sRule_Obj_In_Name + '_OFF@' + sRule_Id,
        'Rule_Obj_In', sRule_Obj_In);
      myRule.WriteString('!' + sRule_Obj_In_Name + '_OFF@' + sRule_Id,
  ...
  begin
    sObj_Id := Copy(sMsg, Pos('#', sMsg) + 1, Pos('*', sMsg) -
        (Pos('#', sMsg) + 1));
    ...
    begin
      ...
      begin
        ...
        NextGrid1.Cell[4, iBaris].AsFloat := myReg.ReadInteger
          ('SETTINGS', 'power' + IntToStr(iBaris + 1), 0)
          * NextGrid1.Cell[3, iBaris].AsInteger / 100;
        // kirim status terbaru objek ke kontrol dan monitoring
        AContext.Connection.IOHandler.WriteLn
          ('#' + sObj_Id + '*' + sData + '&' + NextGrid1.Cell[4, iBaris]
            .AsString);
      end
      else
        Inc(iBaris);
...
var
  ...
  i, iObj_in, iObj_in_status, iPower: Integer;

begin
  myRule := TIniFile.Create(ExtractFilePath(Application.ExeName) + 'Rule.ini');
  myObject := TIniFile.Create(ExtractFilePath(Application.ExeName)
      + 'Settings.ini');
  iBaris := NextGrid1.SelectedRow;
  iObj_in := NextGrid1.Cell[1, iBaris].AsInteger;
  iObj_in_status := NextGrid1.Cell[5, iBaris].AsInteger;
  ...
  begin
    ...
    while i < ruleSectionList.Count do
    begin
      sNamaRule := Copy(ruleSectionList[i], Pos('!', ruleSectionList[i]) + 1,
        Pos('@', ruleSectionList[i]) - 2);
      sIdRule := Copy(ruleSectionList[i], Pos('@', ruleSectionList[i]) + 1,
        MaxInt);
      // proses pencarian rule
      if sNamaRule = sObj_In_Name + '_ON' then
      begin
        sRule_Id_Obj_Out := myRule.ReadString
          ('!' + sObj_In_Name + '_ON@' + sIdRule, 'Rule_Obj_Out', '0');
        sRule_Obj_Out_Status := myRule.ReadString
          ('!' + sObj_In_Name + '_ON@' + sIdRule, 'Rule_Obj_Out_Status', '0');
        iPower := myObject.ReadInteger('SETTINGS', 'power' + sRule_Id_Obj_Out,
          20);
        IdTCPClientUpdate.Socket.WriteLn
          ('!' + sRule_Id_Obj_Out + '@' + sRule_Obj_Out_Status + '#' + IntToStr
            (iPower));
      end
      else
      begin
      ...
      while iNo < NextGrid1.RowCount do
      begin
        // proses pencarian objek dan eksekusi rule
        ...
        begin
          ...
          NextGrid1.Cell[4, iNo].AsInteger := iPower;
  ...
  begin
    ...
    begin
      ...
      sIdRule := Copy(ruleSectionList[i], Pos('@', ruleSectionList[i]) + 1,
        MaxInt);
      // proses pencarian rle
      if sNamaRule = sObj_In_Name + '_OFF' then
      begin
        sRule_Id_Obj_Out := myRule.ReadString
          ('!' + sObj_In_Name + '_OFF@' + sIdRule, 'Rule_Obj_Out', '0');
        sRule_Obj_Out_Status := myRule.ReadString
          ('!' + sObj_In_Name + '_OFF@' + sIdRule, 'Rule_Obj_Out_Status', '0');
        iPower := myObject.ReadInteger('SETTINGS', 'power' + sRule_Id_Obj_Out,
          20) * 0;
        IdTCPClientUpdate.Socket.WriteLn
          ('!' + sRule_Id_Obj_Out + '@' + sRule_Obj_Out_Status + '#' + IntToStr
      ...
      while iNo < NextGrid1.RowCount do
      begin
        // proses pencarian obje dan eksekusi rule
        if NextGrid1.Cell[1, iNo].AsInteger = StrToInt(sRule_Id_Obj_Out) then
        begin
          NextGrid1.Cell[3, iNo].AsInteger := StrToInt(sRule_Obj_Out_Status);
          NextGrid1.Cell[4, iNo].AsInteger := iPower;
  ...
  NextGrid1.Cell[3, iBaris].AsInteger := NextGrid1.Cell[5, iBaris].AsInteger;
  if NextGrid1.Cell[3, iBaris].AsInteger = 100 then
  begin
    NextGrid1.Cell[4, iBaris].AsInteger := myObject.ReadInteger
      ('SETTINGS', 'power' + sRule_Id_Obj_Out, 20);
  end
  else
  begin
    NextGrid1.Cell[4, iBaris].AsInteger := myObject.ReadInteger
      ('SETTINGS', 'power' + sRule_Id_Obj_Out, 20) * 0;
  end;
  IdTCPClientUpdate.Socket.WriteLn
    ('!' + NextGrid1.Cell[1, iBaris].AsString + '@' + NextGrid1.Cell[3, iBaris]
      .AsString + '#' + NextGrid1.Cell[4, iBaris].AsString);
  myObject.WriteInteger('SETTINGS', 'status' + IntToStr(iObj_in),
    NextGrid1.Cell[5, iBaris].AsInteger);
  myRule.Free;
end;
